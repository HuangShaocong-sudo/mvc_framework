...
from mvc_framework.utils import random_string
...
def test():
    ...
    with connection.cursor() as cursor:
        cursor.execute('DROP DATABASE IF EXISTS`framework_db`')
        cursor.execute('CREATE DATABASE `framework_db` CHARACTER SET utf8mb4')
        cursor.execute('USE `framework_db`')
