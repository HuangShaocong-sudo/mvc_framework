...
def index(request):
    ...
    # 替换模板文件中的标记字符串
    return html_response('weibo_index.html', weibos=weibos, token=token)
...
def add(request):
    ...
    u = current_user(request)
    form = request.form()
    form['content'] = remove_script(form['content'])
    Weibo.add(form, u.id)
...
def comment_add(request):
    ...
    owner_id = Weibo.one(id=weibo_id).user_id
    return redirect('/weibo/index?user_id={}'.format(owner_id))
...
def weibo_owner_required(route_function):
    ...
    def f(request):
        log('weibo_owner_required', route_function)
        u = current_user(request)
        id_key = 'weibo_id'
...
def route_dict():
    ...
    d = {
        ...
        '/weibo/delete': login_required(weibo_owner_required(csrf_required(delete))),
